...
int handle_command(char *packet, char *msgtype, char *peer_public_key,
		   char *cmd, device_info_t * di)
{
	char buffer[MAXLINE];
...
int handle_msg(char *packet, device_info_t * di)
{
	...
	if (strlen((const char *)msg.nonce) <= 0) {
		...
		handle_command(packet, msgtype, (char *)msg.public_key,
			       msg.plain_text, di);
