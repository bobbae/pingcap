...
void main()
{
	...
	crypto_sign(signature, secret_key, signature_public_key, unique_id, strlen(unique_id));
	...
	if (crypto_check(signature, signature_public_key, unique_id, strlen(unique_id))) {
	...
	if (crypto_unlock(plain_text, peer_shared_secret, nonce, mac, cipher_text, 12)) {
