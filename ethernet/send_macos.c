...
unsigned char dest_mac[ETHER_ADDR_LEN] = { 0x01, 0x00, 0x5e, 0xaa, 0xbb, 0xcc };

//unsigned char dest_mac[ETHER_ADDR_LEN]  = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
void assoc_dev(int bpf, char *interface)
{
	...
	if (ioctl(bpf, BIOCSETIF, &bound_if) > 0) {
		printf
		    ("Cannot bind bpf device to physical device %s, exiting\n",
		     interface);
		exit(1);
	}
	...
	while (run_loop) {
		...
		if ((read_bytes = read(bpf, bpf_buf, buf_len)) > 0) {
			...
			while (ptr < ((char *)(bpf_buf) + read_bytes)) {
				...
				frame =
				    (struct frame_t *)((char *)bpf_packet +
						       bpf_packet->bh_hdrlen);
				frame->len = bpf_packet->bh_caplen;
				frame->payload_len =
				    frame->len - (2 * ETHER_ADDR_LEN) -
				    ETHER_TYPE_LEN;
				// Do something with the frame
				printf
				    ("Got packet, length of frame: %ld, length of data: %ld\n",
...
void write_single_frame(int bpf)
{
	...
	frame.header.ether_type = 0xacdc;
	//frame.header.ether_type = 0x0800;
...
void write_frames(int bpf, const unsigned char *databuf, size_t datalen)
{
	...
	do {
		...
		printf("Copying payload from %lu, length %lu\n", start,
		       bytes_to_send);
		memcpy(frame->payload, (void *)(databuf + start),
		       bytes_to_send);
		...
		// padding.
		printf
		    ("Total frame length: %lu of maximum ethernet frame length %d\n",
		     frame->len, ETHER_MAX_LEN - ETHER_CRC_LEN);
		bytes_sent = write(bpf, frame, frame->len);
		// Check results
		if (bytes_sent < 0) {
			perror
			    ("Error, perhaps device doesn't have IP address assigned?");
			exit(1);
